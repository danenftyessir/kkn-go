...
class ProjectsSeeder extends Seeder
{
    ...
    {
        // ambil aplikasi yang sudah accepted
        ...
        $this->command->info('Data projects berhasil dibuat!');
    }
    ...
    protected function createMilestones(Project $project)
    {
        ...
        $milestones = [
            ...
            [
                ...
                'progress_percentage' => $project->status === 'completed' ? 100 : rand(0, 70),
                'status' => $project->status === 'completed' ? 'completed' : ($project->progress_percentage > 70 ? 'in_progress' : 'pending'),
                'completed_at' => $project->status === 'completed' ? $project->start_date->copy()->addMonths(floor($duration * 0.85)) : null,
            ],
            [
                'title' => 'Pelaporan dan Penutupan',
                'description' => 'Penyusunan laporan akhir dan serah terima program kepada institusi',
                'order' => 4,
                'target_date' => $project->end_date->copy()->subWeek(),
                'progress_percentage' => $project->status === 'completed' ? 100 : 0,
                'status' => $project->status === 'completed' ? 'completed' : 'pending',
                'completed_at' => $project->status === 'completed' ? $project->end_date : null,
            ],
        ];
     ...
     */
    protected function createReports(Project $project, $count)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            ProjectReport::create([
                ...
                'type' => $type,
                'title' => ucfirst($type === 'weekly' ? "Laporan Mingguan - Minggu ke-{$weekNumber}" : "Laporan Bulanan - Bulan ke-{$weekNumber}"),
                ...
                'photos' => rand(1, 100) <= 60 ? ['reports/photos/photo-1.jpg', 'reports/photos/photo-2.jpg'] : null,
    ...
    protected function createDocument(Project $project)
    {
        ...
        Document::create([
            ...
            'description' => 'Dokumentasi lengkap pelaksanaan program ' . $project->title . ' yang dilaksanakan di ' . $project->problem->regency->name,
            'file_path' => 'documents/final-reports/document-' . $project->id . '.pdf',
            'file_type' => 'pdf',
            'file_size' => rand(1000000, 5000000), // 1-5 MB
            'categories' => $project->problem->sdg_categories,
            'tags' => ['kkn', 'laporan', strtolower($project->problem->regency->name)],
            'author_name' => $student->first_name . ' ' . $student->last_name,
     ...
     */
    protected function getRandomReviewText()
    {
        $reviews = [
            'Mahasiswa sangat berdedikasi dan menunjukkan profesionalisme tinggi dalam pelaksanaan program. Hasil kerja memuaskan dan memberikan dampak positif bagi masyarakat.',
            'Pelaksanaan program berjalan sangat baik. Mahasiswa proaktif, komunikatif, dan mampu beradaptasi dengan kondisi lapangan. Sangat puas dengan hasil kerjanya.',
            'Program terlaksana dengan baik sesuai rencana. Mahasiswa menunjukkan komitmen yang kuat dan mampu mengatasi berbagai kendala yang dihadapi.',
            'Sangat impressed dengan dedikasi dan kerja keras mahasiswa. Program memberikan manfaat nyata bagi masyarakat dan dilaksanakan dengan profesional.',
            'Mahasiswa menunjukkan kemampuan yang baik dalam mengelola program dan berkoordinasi dengan berbagai pihak. Hasil yang dicapai melebihi ekspektasi.',
        ];
