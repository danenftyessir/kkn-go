{{-- 
    komponen card untuk menampilkan problem
    digunakan di halaman browse problems (grid view)
...
<div class="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group">
    ...
    <div class="p-5">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-600 mb-3">
            ...
            <span class="truncate">{{ $problem->location }}</span>
        </div>
        ...
        <div class="flex flex-wrap gap-1.5 mb-4">
            ...
            @foreach($displayCategories as $category)
                <span class="{{ sdg_color($category) }} text-white text-xs font-semibold px-2.5 py-1 rounded-md">
                    SDG {{ substr($category, -2) }}
                </span>
            @endforeach
            
            @if(count($categories) > 2)
                <span class="bg-gray-200 text-gray-700 text-xs font-semibold px-2.5 py-1 rounded-md">
                    +{{ count($categories) - 2 }}
                </span>
            @endif
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
            <div class="flex items-center gap-4 text-sm text-gray-600">
                <div class="flex items-center gap-1">
                    ...
                    <span class="font-medium">{{ $problem->students_needed }}</span>
                </div>
                <div class="flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                    <span class="font-medium">{{ $problem->duration_months }} bln</span>
                </div>
                <div class="flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                    <span class="font-medium">{{ $problem->applications_count ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="mt-3">
            <span class="{{ $difficulty['bg'] }} {{ $difficulty['text'] }} text-xs font-semibold px-2.5 py-1 rounded-md">
                {{ $difficulty['label'] }}
            </span>
        </div>
    </div>
    ...
    <div class="px-5 pb-5">
        <a href="{{ route('student.browse-problems.detail', $problem->id) }}" 
           class="block w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg text-center transition-colors duration-200 shadow-md hover:shadow-lg">
            Lihat Detail
        </a>
    </div>
</div>